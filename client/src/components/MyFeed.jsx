...
function MyFeed({ showNavbar = true }) {
    const { user } = useAuth();
    ...
    if (loading) {
        return (
            <div className={showNavbar ? "min-h-screen relative overflow-hidden" : "w-full"}>
                {showNavbar && (
                ...
                {showNavbar && <Navbar />}
                <div className={showNavbar ? "max-w-3xl mx-auto px-6 py-24" : "w-full p-6"}>
    ...
    return (
        <div className={showNavbar ? "min-h-screen relative overflow-hidden" : "w-full"}>
            ...
            {showNavbar && <Navbar />}
            <div className={showNavbar ? "max-w-3xl mx-auto px-6 py-10" : "w-full"}>
                {/* Header */}
                <div className={showNavbar ? "mb-10" : "mb-6"}>
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className={showNavbar ? "text-3xl font-semibold tracking-tight text-slate-800 dark:text-slate-100" : "text-xl font-semibold tracking-tight text-slate-800 dark:text-slate-100"}>My Feed</h1>
                                <p className={showNavbar ? "text-sm text-slate-600 dark:text-slate-400" : "text-xs text-slate-600 dark:text-slate-400"}>Latest updates from your network</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className={showNavbar ?
                                    "h-11 px-6 rounded-full bg-teal-600 hover:bg-teal-700 dark:bg-teal-500 dark:hover:bg-teal-400 text-white font-medium shadow-sm text-sm flex items-center gap-2 transition-colors" :
                                    "h-9 px-4 rounded-full bg-teal-600 hover:bg-teal-700 dark:bg-teal-500 dark:hover:bg-teal-400 text-white font-medium shadow-sm text-xs flex items-center gap-2 transition-colors"
                                }
                            >
                            ...
                            <button
                                ...
                                className={showNavbar ?
                                    "h-11 px-6 rounded-full bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-400 text-white font-medium shadow-sm text-sm flex items-center gap-2 transition-colors" :
                                    "h-9 px-4 rounded-full bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-400 text-white font-medium shadow-sm text-xs flex items-center gap-2 transition-colors"
                                }
                            >
                ...
                {/* Posts Feed */}
                <div className={showNavbar ? "space-y-6" : "space-y-4"}>
                    {posts.length === 0 ? (
                        <div className={`rounded-2xl ${showNavbar ? 'p-10' : 'p-6'} text-center ${subtleCard}`}>
                            <div className="mb-5">
                                <MessageSquare className={`${showNavbar ? 'h-14 w-14' : 'h-10 w-10'} mx-auto mb-5 text-teal-500/70`} />
                                <h3 className={`${showNavbar ? 'text-xl' : 'text-lg'} font-semibold text-slate-800 dark:text-slate-100 mb-2`}>No posts yet</h3>
                                <p className={`text-slate-600 dark:text-slate-400 ${showNavbar ? 'text-sm' : 'text-xs'} max-w-sm mx-auto leading-relaxed`}>
                        ...
                        posts.map((post) => (
                            <div key={post.id} className={`group rounded-2xl overflow-hidden ${subtleCard} shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5`}>
                                <div className={showNavbar ? "p-6" : "p-4"}>
                                    <div className="flex items-start space-x-3">
                                        {/* User Avatar */}
                                        <div className={`${showNavbar ? 'w-12 h-12' : 'w-10 h-10'} rounded-xl overflow-hidden ring-2 ring-white/60 dark:ring-slate-800/60 shadow`}>
                                            {post.profilePictureBase64 ? (
                                                ...
                                                <div className={`w-full h-full bg-gradient-to-br from-teal-500 via-indigo-500 to-amber-500 flex items-center justify-center text-white font-bold ${showNavbar ? 'text-sm' : 'text-xs'}`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between mb-3">
                                                <div>
                                                    <h3 className={`font-semibold text-slate-800 dark:text-slate-100 ${showNavbar ? 'text-sm' : 'text-xs'}`}>
                                                        {post.profileName || 'Anonymous'}
                                                    </h3>
                                                    <p className={`text-slate-500 dark:text-slate-400 ${showNavbar ? 'text-xs' : 'text-[10px]'}`}>{formatDate(post.createdAt)}</p>
                                                </div>
                                            ...
                                            {/* Post Content */}
                                            <p className={`text-slate-700 dark:text-slate-200 mb-4 leading-relaxed ${showNavbar ? 'text-sm' : 'text-xs'}`}>
                                                {post.content}
                                            </p>
                                                    ...
                                                    {post.images.map((image, index) => (
                                                        ...
                                                        >
                                                            <img
                                                                ...
                                                                className={`w-full ${showNavbar ? 'h-56' : 'h-32'} object-cover transition-transform duration-300 group-hover/image:scale-[1.03] rounded-xl border border-teal-900/10 dark:border-slate-700/60`}
                                                            />
