...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    // Simplified feed query - only returns current user's posts for now
    @Query("SELECT p FROM Post p WHERE p.profile.user = :currentUser ORDER BY p.createdAt DESC")
    List<Post> findPostsFromFriendsAndSelf(@Param("currentUser") User currentUser);
} 